...
mcp = FastMCP("slate-memory", description="Persistent memory storage using SlateDB")
...
class SlateDBMemory:
    ...
    def store(self, key: str, value: Any, metadata: dict[str, Any] | None = None) -> dict[str, Any]:
...
@mcp.tool()
def store_memory(key: str, value: Any, metadata: dict[str, Any] | None = None) -> dict[str, Any]:
    """
...
def delete_memory(key: str) -> dict[str, Any]:
    ...
    if memory.delete(key):
        return {
            "success": True,
            "message": f"Deleted memory with key: {key}"
        }
    else:
        return {
            "success": False,
